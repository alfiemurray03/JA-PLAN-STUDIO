...
const FAQS = [
  ...
  { q: 'Will entering a company name change an Individual account?', a: 'No. Planyx only treats an account as an Organisation when the customer explicitly selects Organisation. A company name alone never mixes the two workspaces.' },
...
function PlanCard({ plan, paymentsEnabled, audience, signedIn }: { plan: ServicePlan; paymentsEnabled: boolean; audience: AccountType; signedIn: boolean }) {
  ...
  return (
    <article className={`relative flex h-full min-w-0 flex-col overflow-hidden rounded-2xl border p-6 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md ${featured ? 'border-primary ring-2 ring-primary/15 bg-primary/[0.03]' : 'border-border bg-card'}`}>
      {featured && <Badge className="absolute -top-3 left-6 max-w-[calc(100%-3rem)]">Planyx package</Badge>}
    ...
    <>
      ...
      <main className="min-h-screen overflow-x-clip bg-background">
        <section className="border-b border-border bg-gradient-to-b from-primary/10 to-background px-4 py-20 text-center">
          <Badge variant="outline" className="mb-5">Planyx plans</Badge>
        ...
        <div className="mx-auto w-full max-w-7xl px-4 py-16 sm:px-6 lg:px-8">
          ...
          {paymentsDisabled && <div className="mb-8 rounded-xl border border-blue-200 bg-blue-50 px-5 py-4 text-sm text-blue-900"><strong>Payments are coming soon.</strong> You can compare every subscription now, but checkout is temporarily switched off by Planyx.</div>}
          {checkoutUnavailable && !paymentsDisabled && <div className="mb-8 rounded-xl border border-amber-200 bg-amber-50 px-5 py-4 text-sm text-amber-900">Checkout is temporarily unavailable. No payment was taken. Please try again later or contact Planyx.</div>}
          ...
          <section className="mb-20"><div className="mb-9 text-center"><Badge variant="outline" className="mb-3">Building your plan</Badge><h2 className="text-3xl font-bold text-foreground">What Planyx helps you build</h2></div><div className="grid gap-5 md:grid-cols-3">{[
