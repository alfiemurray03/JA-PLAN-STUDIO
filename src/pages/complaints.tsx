...
import { GROUP_CONTACT_EMAIL, GROUP_PHONE_DISPLAY, GROUP_PHONE_HREF, PLANYX_EMAIL, normaliseContactDetails } from '@/lib/contact-details';
...
export default function ComplaintsPolicyPage() {
  ...
  useEffect(() => {
    fetch('/api/legal?slug=complaints-policy', { cache: 'no-store' })
    ...
    <main className="min-h-screen bg-background">
      <article className="mx-auto max-w-3xl px-6 py-12 text-foreground">
        <header className="mb-8 flex items-center gap-3">
          ...
          <div><h1 className="text-2xl font-bold">Complaints Policy</h1><p className="text-sm text-muted-foreground">Effective {effectiveDate}{liveContent ? ` · Version ${liveContent.version}` : ' · Version 1.0'}</p></div>
        </header>
        {liveContent ? (
