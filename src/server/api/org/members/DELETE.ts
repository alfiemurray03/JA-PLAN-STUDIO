...
export default async function handler(req: Request, res: Response) {
  try {
    ...
    if (!memberId || typeof memberId !== 'number') return res.status(400).json({ success: false, error: 'memberId is required.' });
    const [myMembership] = await db.select().from(ja_org_members).where(eq(ja_org_members.userId, user.id)).limit(1);
    if (!myMembership || (myMembership.role !== 'owner' && myMembership.role !== 'admin')) return res.status(403).json({ success: false, error: 'Only organisation owners and admins can remove members.' });
    const [target] = await db.select().from(ja_org_members).where(and(eq(ja_org_members.id, memberId), eq(ja_org_members.orgId, myMembership.orgId))).limit(1);
