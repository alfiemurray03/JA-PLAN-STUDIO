...
test("administrator OIDC flow uses state, nonce, PKCE and a validated signed token", async () => {
  ...
  try {
    const start = await beginLogin({
      request: new Request("https://planyx.jagroupservices.co.uk/admin/login?return_to=%2Fadmin%2F%3Fsection%3Dcustomers"),
      env: { ...env, DB }
    }, "admin");
    assert.equal(start.status, 302);
    const authorization = new URL(start.headers.get("location"));
    expectedNonce = authorization.searchParams.get("nonce");
    assert.equal(authorization.searchParams.get("redirect_uri"), "https://planyx.jagroupservices.co.uk/auth/callback");
    ...
    await assert.rejects(() => completeLogin({
      request: new Request(`https://planyx.jagroupservices.co.uk/auth/callback?code=test-code&state=${encodeURIComponent(state)}`, {
